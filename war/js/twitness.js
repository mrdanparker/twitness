...
function showTweets(username){
	...
	var searchedUser = $("<span/>").append($("<a/>").attr({href:"#" + username}).text(username)).addClass("username");
	...
	$.twitnessCacheSearch({from: username}, function(results){
		...
		$('#recent').slideUp(400, function(){
			$('#recent').empty();
			if (results.length == 0) {
				noGraphMsg($("<span>").append("No pushup tweets yet for ").append(searchedUser));
			}
			else {
				...
				if(username && !allPlotData[username]){
						noGraphMsg("Good work on starting the program and tweeting about it.");
				}
...
function noGraphMsg(reason) {
	$('#graph').hide();
	var item = $('<li/>');
	item.append(reason);
	item.append($("<br>")).append("If you tweet in the style shown below we'll graph your progress for you.");
	item.append($('<p id="exampleTweet" class="tweet"/>').append('Passed the first day of #hundredpushups 2,3,2,2,3'));
	$('#recent').prepend(item);
